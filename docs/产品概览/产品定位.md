...
> 文档定位：本文是产品方向的决策分析和论证材料，用于解释 Warehouse 为什么不继续走传统网盘路线，以及为什么要服务 AI / agent 场景。它不是 V2 执行清单。已经收敛的工程边界维护在 [系统架构-V2.md](../产品架构/架构演进/系统架构-V2.md)。
>
> 2026-08-01 决策更新：验证后确认 Agent Run、Context Manifest、Service Principal 和 Artifact Provenance 属于 Knowledge 的上层业务语义。Warehouse 保持文件/对象、权限、凭证、配额、checksum、WebDAV/S3 和复制等通用存储职责；Knowledge 依赖 Warehouse 保存原始资产、manifest 投影和 artifact 文件。本文中早期的 Agent Workspace 控制面建议已被该决策取代，执行边界以 Knowledge 的 `docs/agent-run-context-assets.md` 和 Warehouse 的 [系统架构-V2.md](../产品架构/架构演进/系统架构-V2.md) 为准。

## 0. 结论摘要

### 当前产品定位

Warehouse 是社区用户第一手资料的存储与管理底座。它负责以安全、可靠、可追溯的方式保存和管理用户原始文件及其必要的对象事实，并提供受控的访问、更新、分享、校验和恢复能力。

Knowledge 建立在 Warehouse 之上，负责把这些资料加工成知识或记忆：包括导入、解析、切分、索引、检索、引用、证据和处理过程元数据。Knowledge 可以将 manifest、artifact、反馈文件等加工产物写回 Warehouse，但 Warehouse 不解释这些文件的知识业务语义。

两者的事实边界是：

| 产品 | 负责的事实 | 不负责的事实 |
| --- | --- | --- |
| Warehouse | 文件内容、目录、对象元数据、版本、checksum、权限、凭证、配额、复制和协议访问 | 知识条目、向量索引、检索结果、记忆关系、Agent Run 业务状态 |
| Knowledge | 资料导入、知识加工、索引、检索、context、证据、provenance 和记忆元数据 | 文件系统权限、底层对象持久化和通用存储协议 |

因此，Warehouse 是资料事实源，Knowledge 是知识与记忆事实源；Knowledge 不应复制文件二进制作为自己的长期事实源，Warehouse 也不应把知识业务模型下沉到存储层。

### 面向大模型生态的接入原则

社区所有产品都应能被大模型、Chat 或 Agent 快速、安全地接入，但不要求每个产品各自建设模型网关。职责应保持分层：

| 层次 | 责任 | Warehouse 的要求 |
| --- | --- | --- |
| 模型访问 | Router 统一提供模型渠道、额度、计量和模型调用 | 不在 Warehouse 内重复实现模型调用或模型密钥管理 |
| 产品能力 | 各产品提供稳定、可版本化的业务接口和清晰的数据契约 | 提供对象、权限、配额和访问状态的稳定 API 与协议契约 |
| AI 工具接入 | Chat、Agent 或应用将产品能力封装为受控 Tool / MCP 等调用形态 | 工具必须复用 Warehouse 的权限、路径范围、配额和审计规则，不能绕过存储数据面 |
| 用户授权 | Wallet / Node 与业务产品确认身份和授权依据 | 使用短期、可撤销、范围受限的凭证，不向 AI 客户端暴露长期用户密钥 |

对 Warehouse 而言，快速融入大模型生态的当前基础是 OpenAPI、HTTP 对象接口、WebDAV 和 S3 协议；它们让 Knowledge、Chat、Agent 和第三方应用能够以现有工具链读写受权资料。面向 Agent 的 Tool / MCP 封装属于后续接入形态，应建立在这些稳定接口之上，并以最小权限、可审计和可撤销为前提，不能把 MCP 误写为当前已实现能力。

默认路径应是“AI 应用或 Agent 经用户授权调用 Warehouse”，而不是“模型直接拥有 Warehouse 全局权限”。需要资料理解时，优先由 Knowledge 将受控资料转为带证据的 context；需要文件或产物读写时，再以限定目录和动作范围访问 Warehouse。
...
- Warehouse 的 V2 执行项维护在 [系统架构-V2.md](../产品架构/架构演进/系统架构-V2.md)，本文不再单独维护开发清单。
...
建议立即进入“Warehouse 支撑 Knowledge 的数据源与反馈资产验证阶段”。Knowledge 负责 agent 上层语义，Warehouse 负责稳定资产存储和协议接入；两者的开发边界以 [系统架构-V2.md](../产品架构/架构演进/系统架构-V2.md) 为准。
