...
- `/api/v1/public/auth/email/code` 只保留本地验证码登录入口；Warehouse 不再直接通过 SMTP 发送验证码邮件。
...
- 邮件发送能力统一收敛到 Node；身份邮箱验证优先使用 Node Identity 的邮箱验证流程。

### 钱包身份登录

- `/api/v1/public/auth/identity/login/session` 由 Warehouse 本地生成登录会话、`audience`、`nonce` 和所需 scopes，不依赖 Node 服务在线。
- `/api/v1/public/auth/identity/login/verify` 接收钱包返回的 presentation，在 Warehouse 本地校验 presentation 签名、身份文档签名、钱包地址证明和 JWT-VC 凭证。
- 本地 JWT-VC 校验读取 `identity.identity_trust_dir`。
- trust bundle 目录需要包含 `issuer-metadata.json`、`jwks.json`、`manifest.json`；`manifest.json` 中的 `metadataSha256` / `jwksSha256` 必须与文件内容匹配。
- Identity 扫码登录通过 `identity.node_url` 调用 Node 的授权请求、状态查询和授权码交换接口。
