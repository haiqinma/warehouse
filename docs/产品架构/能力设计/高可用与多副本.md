...
本文合并原来的 active/standby、内部复制备用节点、容灾和多实例路线内容，作为 Warehouse 多副本相关的唯一设计入口。部署命令和操作步骤仍以 [部署手册.md](../../运维部署/部署手册.md) 为准。
...
`--dry-run` 只统计当前保留期下可删除的 item、job 和 outbox 数量。人工执行不提供绕过 current generation、running job 或 assignment last job 保护的 `--force`。

## 运维入口

多副本部署、升级、回滚命令见[部署手册](../../运维部署/部署手册.md)，切换和排障命令见[运维手册](../../运维部署/运维手册.md)。本文不重复命令，只解释方案。
