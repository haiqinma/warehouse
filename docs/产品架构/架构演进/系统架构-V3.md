...
当前实现基线由 [系统架构-V1.md](系统架构-V1.md) 和 [系统架构-V2.md](系统架构-V2.md) 共同描述。
...
4. 使接收列表、详情、共享目录浏览，以及已提供的创建目录、上传、重命名、删除操作切到 resource + effective grant 授权。
5. 收件端不再读取或传递 legacy `shareId`；所有仍未迁移的能力必须保持不可用，不能回落到旧路由。
6. 完成回归测试和迁移后的生产验证后，再按 owner 管理路径的迁移进度移除旧路由和表依赖。

服务启动会在 schema migration 后、开始接流量前自动执行带 PostgreSQL advisory lock 的 V3 回填和核对。接收端 `/share/user/received` 已按 resource 聚合；目录枚举、下载、创建目录、上传会话、在线文本编辑、重命名、删除和拖拽移动均按 V3 effective grant 对每个请求重新授权，并由 mutation recorder 记录底层变更。移动/删除还会同步 owner 名下受影响的 V3 resource 路径或清理其 grant/audience，不能依赖下次重启 reconcile 才恢复一致性。二次创建公开链接没有安全的 resource-scoped 语义，相关 legacy API 已移除。以下 CLI 仍用于预演、人工复核和故障排查：
...
详细交互、索引、兼容与接口设计见 [共享方案.md](../能力设计/共享与授权.md)。
