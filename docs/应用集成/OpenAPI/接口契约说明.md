...
- 独立 S3 Endpoint 的 Signature V4、bucket/key 和兼容操作由 [S3 设计方案](../S3协议.md) 维护。
...
`/api/v1/internal/*` 内部复制接口按设计不进入对外规范；WebDAV 协议操作由用户使用指南维护，独立 S3 Endpoint 的协议操作由 [S3 设计方案](../S3协议.md) 维护。新增或修改对外 HTTP API 时，必须同步修改 OpenAPI。
...
```bash
npx --yes @redocly/cli bundle docs/应用集成/OpenAPI/warehouse.openapi.yaml \
  --output docs/应用集成/OpenAPI/warehouse.openapi.bundle.yaml
```

本地预览：

```bash
npx --yes @redocly/cli preview-docs docs/应用集成/OpenAPI/warehouse.openapi.yaml
```
